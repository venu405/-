      ...
      <section class="panel panel-form panel-centered">
        ...
        <form class="form" @submit.prevent="handleSubmit">
          ...
          <section class="options">
            ...
            <label class="field option">
              <span>研究深度</span>
              <select v-model.number="form.depth">
                <option :value="1">快速（1 轮）</option>
                <option :value="2">标准（2 轮）</option>
                <option :value="3">深度（3 轮）</option>
              </select>
            </label>
          </section>
        ...
        <div class="tasks-section" v-if="todoTasks.length">
          ...
          <article class="task-detail" v-if="currentTask">
            <header class="task-header">
              ...
              <button
                type="button"
                class="chip-action regenerate-btn"
                :disabled="loading || !currentTask"
                @click="regenerateTask(currentTask.id)"
              >
                {{ loading ? "重新生成中…" : "重新生成" }}
              </button>
            </header>
...
const form = reactive({
  topic: "",
  searchApi: "",
  depth: 2 // P1: 研究深度 1=快速 2=标准 3=深度
});
...
const handleSubmit = async () => {
  ...
  const payload = {
    ...
    research_depth: form.depth || undefined // P1: 研究深度覆盖后端轮数上限
  };
...
const regenerateTask = async (taskId: number) => {
  // =D4 改造③= 任务级"重新生成"：只重跑一个任务，报告用新总结更新
  if (!form.topic.trim() || loading.value) {
    return;
  }
  error.value = "";
  loading.value = true;

  // 把当前所有任务的数据发回后端，后端据此重建状态
  const tasksPayload = todoTasks.value.map((task) => ({
    id: task.id,
    title: task.title,
    intent: task.intent,
    query: task.query,
    ...
    sources_summary: task.sourcesSummary,
    note_id: task.noteId,
    note_path: task.notePath
  }));
  ...
  try {
    await runResearchStream(
      ...
      (event: ResearchStreamEvent) => {
        ...
        if (event.type === "task_summary_chunk") {
          const task = findTask(event.task_id);
          if (task && typeof event.content === "string") {
            task.summary += event.content; // 打字机效果
          }
          return;
        }
        ...
        if (event.type === "task_status") {
          ...
          const status =
            typeof event.status === "string" ? event.status : task.status;
          task.status = status;
          if (status === "in_progress") {
            task.summary = "";
          }
          if (status === "completed" && typeof event.summary === "string") {
            task.summary = event.summary;
            progressLogs.value.push(`任务已完成：${task.title}`);
          }
          return;
        }
        ...
        if (event.type === "final_report") {
          ...
          pulse(reportHighlight); // P2: 报告更新高亮提示
          progressLogs.value.push("报告已基于新总结更新");
          return;
        }

        if (event.type === "error") {
          error.value =
            typeof event.detail === "string" ? event.detail : "重新生成失败";
        }
      },
      { signal: controller.signal },
      "/research/regenerate"
    ...
    error.value =
      err instanceof Error ? err.message : "重新生成任务失败";
  } finally {
    loading.value = false;
    if (currentController === controller) {
      currentController = null;
