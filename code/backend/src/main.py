...
def create_app() -> FastAPI:
    ...
    def regenerate_task(payload: RegenerateRequest) -> StreamingResponse:
    ...
    def kb_ask(payload: KbAskRequest = Body(...)) -> Dict[str, Any]:
        ...
        try:
            ...
            if payload.user_id and not kb["auth"].can_access(payload.user_id, payload.kb_id):
                raise HTTPException(
                    status_code=403,
                    detail=f"用户 {payload.user_id} 无权访问知识库 {payload.kb_id}",
                )
            from services.kb import qa_graph
    ...
    @app.get("/kb/docs")
    def kb_list_docs(kb_id: str | None = None) -> Dict[str, Any]:
    ...
    @app.get("/kb/kbs")
    def kb_list_kbs() -> Dict[str, Any]:
        """列出所有知识库（kb_id 去重）——知识库管理界面用。"""
        try:
            kb = _get_kb()
            return {"kbs": kb["store"].list_kbs()}
        except Exception as exc:
            logger.error("KB list kbs failed: {}", exc)
            raise HTTPException(status_code=500, detail=f"查询失败: {exc}") from exc

    @app.delete("/kb/docs/{doc_id}")
    def kb_delete_doc(doc_id: str) -> Dict[str, Any]:
        """删除文档及其全部分块。"""
        try:
            kb = _get_kb()
    ...
    @app.post("/kb/users")
    def kb_create_user(name: str = Form(...)) -> Dict[str, Any]:
        """新建用户，返回 user_id。"""
        try:
            kb = _get_kb()
            uid = kb["auth"].create_user(name)
    ...
    @app.get("/kb/users")
    def kb_list_users() -> Dict[str, Any]:
        """列出所有用户及其可访问的知识库。"""
        try:
            kb = _get_kb()
    ...
    def kb_get_user(user_id: str) -> Dict[str, Any]:
        """用户详情（含可访问的 kb 列表）。"""
        try:
            kb = _get_kb()
    ...
    def kb_revoke_access(user_id: str, kb_id: str = Form(...)) -> Dict[str, Any]:
        """撤销用户对某知识库的访问权。"""
        try:
            kb = _get_kb()
